...
from app.modules.projects.project_model import Project as Model
...
class ProjectRepo(BaseRepo):
    ...
    async def list(self, db: Session, request: Request):
        ...
        search_fields = ['id', 'name', 'description', 'skill_id']
    ...
    def repo_specific_filters(self, query, Model, query_params):
        ...
        value = query_params.get('skill_id', None)
        if value is not None and value.isdigit():
            query = query.filter(Model.skill_id == int(value))
    ...
    async def create(self, db: Session, model_request):
        required_fields = ['name', 'description', 'skill_id']
        unique_fields = []
        ...
        db_query = Model(
            ...
            skill_id = model_request.skill_id,
    ...
    async def update(self, db: Session, model_id: int, model_request):
        ...
        if db_query:
            ...
            db_query.skill_id = model_request.skill_id
