...
from app.modules.api.flats.flat_repo import FlatRepo as Repo
from app.modules.api.flats.flat_schema import FlatSchema as ModelSchema
...
def view_route(model_id: int, db: Session = Depends(get_db)):
    ...
    if result is None:
        raise HTTPException(status_code=404, detail=f"Flat not found")
    return result
...
# Update statuses of multiple Flats.
@router.put("/statuses")
