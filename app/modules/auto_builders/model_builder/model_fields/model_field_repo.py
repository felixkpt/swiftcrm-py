...
from app.modules.auto_builders.model_builder.model_fields.model_field_model import AutoBuildersModelBuilderModelField as Model
...
class ModelFieldRepo(BaseRepo):
    ...
    async def list(self, db: Session, request: Request):
        ...
        search_fields = ['model_builder_id', 'name', 'type', 'dataType', 'isVisibleInList', 'isVisibleInSingleView']
    ...
    async def create(self, db: Session, model_request):
        required_fields = ['model_builder_id', 'name', 'type', 'dataType', 'isVisibleInList', 'isVisibleInSingleView']
        unique_fields = []
        ...
        db_query = Model(
            ...
            type = str(model_request.type).strip(),
            label = str(model_request.label).strip(),
            dataType = str(model_request.dataType).strip(),
            defaultValue = str(model_request.defaultValue).strip(),
            ...
            dropdownSource = str(model_request.dropdownSource).strip(),
    ...
    async def update(self, db: Session, model_id: int, model_request):
        ...
        if db_query:
            ...
            db_query.type = str(model_request.type).strip()
            db_query.label = str(model_request.label).strip()
            db_query.dataType = str(model_request.dataType).strip()
            db_query.defaultValue = str(model_request.defaultValue).strip()
            ...
            db_query.dropdownSource = str(model_request.dropdownSource).strip()
