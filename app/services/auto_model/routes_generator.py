...
@router.get("/{{{model_name.lower()}_id}}")
...
@router.put("/{{{model_name.lower()}_id}}", response_model=schema.{model_name}Schema)
def update_{model_name.lower()}({model_name.lower()}_id: int, {model_name.lower()}: schema.{model_name}Schema, db: Session = Depends(get_db)):
    db_{model_name.lower()} = model.get_{model_name.lower()}(db, {model_name.lower()}_id={model_name.lower()}_id)
    if db_{model_name.lower()} is None:
        raise HTTPException(status_code=404, detail="{model_name} not found")
    return model.update_{model_name.lower()}(db=db, {model_name.lower()}_id={model_name.lower()}_id, {model_name.lower()}={model_name.lower()})

@router.delete("/{{{model_name.lower()}_id}}")
def delete_{model_name.lower()}({model_name.lower()}_id: int, db: Session = Depends(get_db)):
    db_{model_name.lower()} = model.get_{model_name.lower()}(db, {model_name.lower()}_id={model_name.lower()}_id)
    if db_{model_name.lower()} is None:
        raise HTTPException(status_code=404, detail="{model_name} not found")
    return model.delete_{model_name.lower()}(db=db, id={model_name.lower()}_id)
"""
