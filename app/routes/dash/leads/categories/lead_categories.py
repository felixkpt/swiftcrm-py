...
from app.repositories.dash.leads.categories.lead_category_repo import LeadCategoryRepo as Repo
from app.requests.schemas.dash.leads.categories.lead_category import LeadCategorySchema as ModelSchema
...
def view_route(model_id: int, db: Session = Depends(get_db)):
    ...
    if result is None:
        raise HTTPException(status_code=404, detail="Lead_category not found")
    return result
