...
from app.repositories.string.string_repo import StringRepo as Repo
from app.requests.schemas.string.string import StringSchema as ModelSchema
...
def view_route(model_id: int, db: Session = Depends(get_db)):
    ...
    if result is None:
        raise HTTPException(status_code=404, detail=f"String not found")
    return result

# Update an existing String by ID.
...
# Update statuses of multiple Strings.
@router.put("/statuses")
