...
from app.repositories.social_media.conversation.messages.word_confidences.word_confidence_repo import WordConfidenceRepo as Repo
from app.requests.schemas.social_media.conversation.messages.word_confidences.word_confidence_schema import WordConfidenceSchema as ModelSchema
...
# Create a new Word_confidence instance.
...
# Retrieve counts or statistics related to Word_confidences.
@router.get("/counts")
async def counts_route(request: Request, db: Session = Depends(get_db)):
    results = await repo.list(db, request)
    return results.metadata.total_counts

# Update the status of a Word_confidence by ID.
...
# Update statuses of multiple Word_confidences.
@router.put("/statuses")
...
# Delete a Word_confidence by ID.
