...
from app.repositories.ProjectHistory.projecthistory_repo import ProjecthistoryRepo as Repo
from app.requests.schemas.ProjectHistory.projecthistory_schema import ProjecthistorySchema as ModelSchema
...
# Retrieve counts or statistics related to Projecthistories.
@router.get("/counts")
async def counts_route(request: Request, db: Session = Depends(get_db)):
    results = await repo.list(db, request)
    return results.metadata.total_counts

# Update the status of a Projecthistory by ID.
...
# Update statuses of multiple Projecthistories.
@router.put("/statuses")
