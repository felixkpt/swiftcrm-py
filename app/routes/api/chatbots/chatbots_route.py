...
from app.repositories.api.chatbots.chatbot_repo import ChatbotRepo as Repo
from app.requests.schemas.api.chatbots.chatbot_schema import ChatbotSchema as ModelSchema
...
def view_route(model_id: int, db: Session = Depends(get_db)):
    ...
    if result is None:
        raise HTTPException(status_code=404, detail=f"Chatbot not found")
    return result

# Update an existing Chatbot by ID.
...
# Update statuses of multiple Chatbots.
@router.put("/statuses")
