...
from app.repositories.api.animals.animal_repo import AnimalRepo as Repo
from app.requests.schemas.api.animals.animal_schema import AnimalSchema as ModelSchema
...
def view_route(model_id: int, db: Session = Depends(get_db)):
    ...
    if result is None:
        raise HTTPException(status_code=404, detail=f"Animal not found")
    return result
...
# Update statuses of multiple Animals.
@router.put("/statuses")
