...
from app.repositories.conversation.v2.interviews.interview_repo_old import InterviewRepo as Repo
...
@router.get("/interview/{sub_cat_id}/progress")
...
@router.get("/interview/results/categories", response_model=Dict[str, Any])
async def route_list_interviews_by_categories():
    return Repo.list_all_interviews()


@router.get("/interview/results/categories/{category_id}", response_model=Dict[str, Any])
async def route_list_interviews_by_category_id(category_id: int):
    return Repo.list_interviews_by_category_id(category_id)


@router.get("/interview/results/categories/{category_id}/sub-category/{sub_category_id}", response_model=Dict[str, Any])
