...
class ConversationRepo:
    ...
    def archive_messages(cat_id: int, sub_cat_id: int = None, mode='training'):
        if sub_cat_id:
            query = "UPDATE conversation_v2_messages SET status_id = 0 WHERE sub_category_id = %s AND mode = %s"
            execute_query(query, (sub_cat_id, mode))
        else:
            query = "UPDATE conversation_v2_messages SET status_id = 0 WHERE category_id = %s AND mode = %s"
    ...
    def reset_messages(cat_id, sub_cat_id=None):
        if sub_cat_id:
            query = "DELETE FROM conversation_v2_messages WHERE sub_category_id = %s"
            execute_query(query, (sub_cat_id,))
        else:
            query = "DELETE FROM conversation_v2_messages WHERE category_id = %s"
            execute_query(query, (cat_id,))

    @staticmethod
    def add_message(user_id, cat_id, sub_cat_id, role, content, audio_uri, mode='training'):
        query = "INSERT INTO conversation_v2_messages (user_id, category_id, sub_category_id, role, content, audio_uri, mode) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    ...
    def get_recent_messages(db, sub_cat_id, mode='training'):
        ...
        query = """
        SELECT content, role FROM conversation_v2_messages 
