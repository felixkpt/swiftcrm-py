...
from app.models.conversation_app.categories.sub_categories.questions.question import ConversationAppCategoriesSubCategoriesQuestion as Model
...
class QuestionRepo:
    ...
    async def list(db: Session, request: Request):
        ...
        search_fields = ['conversation_app_category_id', 'conversation_app_categories_sub_category_id', 'question', 'marks']

        query = db.query(Model)
        query = apply_search_and_sort(query, Model, search_fields, query_params)

        value = query_params.get('conversation_app_category_id', None)
        if value is not None:
            query = query.filter(Model.conversation_app_category_id == value)
        value = query_params.get('conversation_app_categories_sub_category_id', None)
        if value is not None:
            query = query.filter(Model.conversation_app_categories_sub_category_id == value)
    ...
    def create(db: Session, model_request):
        required_fields = ['conversation_app_category_id', 'conversation_app_categories_sub_category_id', 'question', 'marks']
        unique_fields = []
        ...
        db_query = Model(
            ...
            conversation_app_category_id=model_request.conversation_app_category_id,
            conversation_app_categories_sub_category_id=model_request.conversation_app_categories_sub_category_id,
    ...
    def update(db: Session, model_id: int, model_request):
        ...
        if db_query:
            ...
            db_query.conversation_app_category_id = model_request.conversation_app_category_id
            db_query.conversation_app_categories_sub_category_id = model_request.conversation_app_categories_sub_category_id
