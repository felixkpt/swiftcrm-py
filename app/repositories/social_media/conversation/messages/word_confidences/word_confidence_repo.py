...
from app.models.social_media.conversation.messages.word_confidences.word_confidence_model import SocialMediaConversationMessagesWordConfidence as Model
...
class WordConfidenceRepo(BaseRepo):
    ...
    async def list(self, db: Session, request: Request):
        ...
        search_fields = ['message_id', 'word', 'start_time_seconds', 'start_time_nanos', 'end_time_seconds', 'end_time_nanos', 'confidence']
    ...
    def repo_specific_filters(self, query, Model, query_params):
        ...
        value = query_params.get('start_time_nanos', '').strip()
        if isinstance(value, str) and len(value) > 0:
            query = query.filter(Model.start_time_nanos.ilike(f'%{value}%'))
        value = query_params.get('end_time_seconds', '').strip()
        if isinstance(value, str) and len(value) > 0:
            query = query.filter(Model.end_time_seconds.ilike(f'%{value}%'))
        value = query_params.get('end_time_nanos', '').strip()
        if isinstance(value, str) and len(value) > 0:
            query = query.filter(Model.end_time_nanos.ilike(f'%{value}%'))
    ...
    async def create(self, db: Session, model_request):
        required_fields = ['message_id', 'word', 'start_time_seconds', 'start_time_nanos', 'end_time_seconds', 'end_time_nanos', 'confidence']
        unique_fields = []
        ...
        db_query = Model(
            ...
            word = str(model_request.word).strip(),
            start_time_seconds = model_request.start_time_seconds,
            start_time_nanos = str(model_request.start_time_nanos).strip(),
            end_time_seconds = model_request.end_time_seconds,
            end_time_nanos = model_request.end_time_nanos,
    ...
    async def update(self, db: Session, model_id: int, model_request):
        ...
        if db_query:
            ...
            db_query.word = str(model_request.word).strip()
            db_query.start_time_seconds = model_request.start_time_seconds
            db_query.start_time_nanos = str(model_request.start_time_nanos).strip()
            db_query.end_time_seconds = model_request.end_time_seconds
            db_query.end_time_nanos = model_request.end_time_nanos
