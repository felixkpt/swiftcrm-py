...
from app.models.social_media.conversation.categories.sub_categories.questions.question_model import SocialMediaConversationCategoriesSubCategoriesQuestion as Model
...
class QuestionRepo(BaseRepo):
    ...
    def repo_specific_filters(self, query, Model, query_params):
        ...
        value = query_params.get('question', '').strip()
        if isinstance(value, str) and len(value) > 0:
            query = query.filter(Model.question.ilike(f'%{value}%'))
    ...
    async def create(self, db: Session, model_request):
        ...
        db_query = Model(
            ...
            question = str(model_request.question).strip(),
    ...
    async def update(self, db: Session, model_id: int, model_request):
        ...
        if db_query:
            ...
            db_query.question = str(model_request.question).strip()
