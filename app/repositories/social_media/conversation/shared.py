...
class SharedRepo:
    ...
    async def get_interview_question(db, request, sub_cat_id, user_id, update=False):
        ...
        question_number, interview, is_completed = SharedRepo.process_interview_questions(
            cat_id, sub_cat_id, questions, update, user_id)

        interview_id = None
        question = None
        if interview and interview.get('current_question_id', False):
            question = QuestionRepo().get(db, interview['current_question_id'])
    ...
    @staticmethod
    def fetch_interview_session(user_id, cat_id, sub_cat_id):
        query_session = """
        SELECT id, current_question_id 
        FROM social_media_conversation_interviews 
        WHERE user_id = %s AND category_id = %s AND sub_category_id = %s AND status_id = %s
        """
        return execute_query(query_session, (user_id, cat_id, sub_cat_id, 1), fetch_method='first')

    @staticmethod
    def process_interview_questions(cat_id, sub_cat_id, questions, update, user_id,):
        ...
        if not interview:
            ...
            if next_question_id and update:
                ...
                execute_query(query_update_session,
                              (next_question_id, datetime.now(), interview['id']))
                interview['current_question_id'] = next_question_id

        return question_number, interview, is_completed
