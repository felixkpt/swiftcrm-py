...
class LeadRepo:
    ...
    async def list(db: Session, request: Request):
        ...
        search_fields = ['dash_leads_categories_lead_category_id', 'comments']

        query = db.query(Model)
        query = apply_search_and_sort(query, Model, search_fields, query_params)

        value = query_params.get('dash_leads_categories_lead_category_id', None)
        if value is not None:
            query = query.filter(Model.dash_leads_categories_lead_category_id == value)
    ...
    def create(db: Session, model_request):
        required_fields = ['dash_leads_categories_lead_category_id', 'comments']
        unique_fields = []
        ...
        db_query = Model(
            ...
            dash_leads_categories_lead_category_id=model_request.dash_leads_categories_lead_category_id,
            comments=model_request.comments,
        )
        db.add(db_query)
        try:
            db.commit()
    ...
    def update(db: Session, model_id: int, model_request):
        ...
        if db_query:
            ...
            db_query.dash_leads_categories_lead_category_id = model_request.dash_leads_categories_lead_category_id
            db_query.comments = model_request.comments
            db.commit()
            db.refresh(db_query)
            return db_query
        else:
