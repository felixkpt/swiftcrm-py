...
from app.models.admin.auto_builders.model_builder.action_labels.action_label import AdminAutoBuildersModelBuilderActionLabel as Model
...
class ActionLabelRepo(BaseRepo):
    ...
    async def list(self, db: Session, request: Request):
        ...
        search_fields = ['model_builder_id', 'key', 'actionType']
    ...
    def repo_specific_filters(self, query, Model, query_params):
        ...
        value = query_params.get('actionType', '').strip()
        if isinstance(value, str) and len(value) > 0:
            query = query.filter(Model.actionType.ilike(f'%{value}%'))
        value = query_params.get('show', '').strip()
        if isinstance(value, str) and len(value) > 0:
            query = query.filter(Model.show.ilike(f'%{value}%'))

        return query

    async def create(self, db: Session, model_request):
        required_fields = ['model_builder_id', 'key', 'actionType']
        unique_fields = []
        ...
        db_query = Model(
            ...
            actionType=model_request.actionType,
            show=model_request.show,
        )
        db.add(db_query)
        try:
            db.commit()
    ...
    async def update(self, db: Session, model_id: int, model_request):
        ...
        if db_query:
            ...
            db_query.actionType = model_request.actionType
            db_query.show = model_request.show
            db.commit()
            db.refresh(db_query)
