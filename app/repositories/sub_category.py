...
from app.requests.schemas.sub_category import QuestionRequest
...
class SubCategoryRepo:
    ...
    def get_sub_cat_questions(sub_cat_id):
        ...
        metadata = {
            'title': '{} questions list'.format(SubCategoryRepo.get_sub_cat(sub_cat_id)['name']) if results else None,
            'total_count': len(results) if results else 0
        }
